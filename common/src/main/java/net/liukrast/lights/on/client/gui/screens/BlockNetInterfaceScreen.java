...
import net.liukrast.lights.on.client.gui.screens.util.ActionBarButton;
import net.liukrast.lights.on.client.gui.screens.util.ActionBarSubButton;
...
public class BlockNetInterfaceScreen extends Screen {
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button) {
        if(mouseY >= (height>>1)-getHalfWindowHeight() && mouseY < (height>>1)-getHalfWindowHeight()+10) {
            if(mouseX >= (width>>1)+getHalfWindowWidth()-30 && mouseX < (width>>1)+getHalfWindowWidth()-20) {
                Services.PACKET_HELPER.send2S(new EjectDiskPacket(blockEntity.getBlockPos()));
                this.onClose();
                return true;
            } else if(mouseX >= (width>>1)+getHalfWindowWidth()-20 && mouseX < (width>>1)+getHalfWindowWidth()-10) {
                fullScreen = !fullScreen;
                return true;
            } else if(mouseX >= (width>>1)+getHalfWindowWidth()-10 && mouseX < (width>>1)+getHalfWindowWidth()) {
                this.onClose();
                return true;
            }
        }
